...
- [ ] Login/auth modes must be driven by runtime config (CONFIG_UPDATE) instead of build-time `appConfig.authModes`; remove client-generated "offline admin" session path and surface 2FA/password-change enforcement with actual enrollment flows (TOTP/WebAuthn). Also ensure default-admin password change/2FA are enforced server-side.
- [ ] Dashboard/status bar: add org/user/role chips; show client↔syncserver RTT, phone presence/RTT, e2e latency p50/p95; keep WS error/reconnect counters. Dashboard needs phone-mockup layout + side panel (filters, quick stats, latency readouts), device badges, claimed-by labels/grey state, smart code copy (prefer `parsed_code`), and WS paging while connected (REST fallback only when degraded).
- [ ] Devices view should stream presence/RTT/SIM-number updates via WS (or polling), and expose ingest-device/number mappings plus detail view; add QR pairing watcher/completion UX, multi-SIM display, and diagnostics cards.
- [ ] Numbers/Users pages must show required fields: ingest-capable devices per number, contact-name mapping, shared/default-device validation; users need last-login, failed-login count, assigned numbers/devices, AD/LDAP mapping details, and in-UI assignments management (role change, disable/enable, unlock/force logout, password reset when allowed).
- [ ] Audit/Logins pages need full filters/columns per spec (actor/action/target/result reason codes, session id, auth mode, device/number, geo/ASN) and expandable detail rows beyond current minimal filters; CSV/JSON export parity.
- [ ] Config console should expose structured sections (Auth modes, RBAC mapping, WS/backfill, retention/Redis fallback, smsrelay3 policies, contact sync) with client-side schema validation and diff/help UI instead of raw textarea edits; apply CONFIG_UPDATE live with banners.
- [ ] Contacts sync UI: enable/disable, last import status, conflict resolution actions, export/download; handle CONTACT_UPDATE to keep view live.
- [ ] Locale UX: expose locale switcher, persist preference, keep lang attribute/SSR consistent with stored selection.
